...
def deep_feature (model, loader, cfg, device, mode):
    ...
    if hasattr (model, 'predictor'):
        ...
        try: handle = model.decoder.rnn.register_forward_hook(get_act)
        except: handle = model.decoder.decoder1.rnn.register_forward_hook(get_act)
        ...
        try: handle = model.decoder.avg.register_forward_hook(get_act)
        except: handle = model.decoder.decoder1.avg.register_forward_hook(get_act)
...
def dim_red (activations, mode, cfg, non_imaging=False, ncols=2):
    ...
    '''
    label_dir, root_dir = cfg ['label_dir'][0], cfg ['root']
    ...
    labs = pd.read_csv (label_path, index_col=[0])
    labs = labs [~pd.isna(labs[cfg ['outcome_col']])]
    ...
    if contain_na ==0:
        ...
        for i, fea in enumerate (cfg['y_features']):
            p = (pn.ggplot (labs, pn.aes (x='PC1', y='PC2', color=fea))+
                    pn.geom_point (size=2)+pn.theme_bw ())
