...
def get_images (loader, loader_set, model, cfg, device, show_maps=None,
        accuracy='least'):
    '''
    ...
    Args:
        ...
        'most' otherwise
    '''
    if show_maps is None: show_maps = VS.show_levels 
    model.eval ()
    ...
    for i in range (cfg['show_num']):
        ...
        if type(img)==tuple or type(img) == list: img = img[0]
        show_maps (np.array (img.permute (2,3,1,0).squeeze(-1)), 
                fig_dir= fig_dir)
