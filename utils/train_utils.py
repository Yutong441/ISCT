...
from utils.loss import get_loss_fun, add_L2, add_L1
from utils.metric import reverse_one_hot, ordinal2label
import CNN.all_models as am
...
def train_loop (model, cf, loaders):
    ...
    L2 = cf['L2'] if len(cf['regularised_layers'])==0 else 0
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, 
        model.parameters()), lr=cf['lr'], weight_decay=L2)
    ...
    for epoch in range(cf['num_epochs']):
        ...
        for imgs, labels in loaders[0]:
            ...
            outputs = model(imgs)
            loss = criterion(outputs, labels) + add_L2 (cf, model)+ \
            add_L1 (cf, model)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
def log_config(filename, cfg):
    with open(filename, 'a') as f:
        ...
        f.write("\n================================================\n")
    with open(filename+'.json', 'w') as f: json.dump (cfg, f)

def train_test (cg):
    model = am.choose_models (cg.config)
    if cg.config ['pretrained']: lw.load_any_net (model, cg.config)
    if os.path.exists (cg.config ['old_save_path']):
        lw.load_CNN_weight (model, cg.config)
    if not torch.cuda.is_available (): cg.config['device'] = 'cpu'

    loader_arg, data_arg = cg.get_data_args(cg.config)
    train_set = dataloader (cg.config['root'], "train", **loader_arg[0],
            transformation=cg.config['transform'])
    eval_set = dataloader (cg.config['root'], "train", transformation=None,
            **loader_arg[0])
    test_set = dataloader (cg.config['root'], "test", **loader_arg[0])
