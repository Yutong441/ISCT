...
class RNN_decoder (DE.CNN_decoder):
    def __init__(self, in_channels, n_classes, step=4, n_decoder=2):
        assert n_decoder >=2, 'number of decoder layers must be at least 2'
        ...
        self.rnn = nn.GRU(in_channels, in_channels//step, bias=True,
                batch_first=False, dropout=0, num_layers=2)
        self.hidden = BB.multi_linear (in_channels//step, 
...
class resCRNN (nn.Module):
    def __init__(self, in_channels, n_classes, add_sigmoid='none', 
            times_max=1, step=4, deepths=[2,2,2,2], extra_features=0,
            n_decoder=2, block=ResNetBasicBlock, *args, **kwargs):
        super().__init__()
